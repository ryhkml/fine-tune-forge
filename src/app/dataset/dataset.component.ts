...
export class DatasetComponent implements OnInit, OnDestroy {
	...
	private initDatasetSubscription() {
		...
		if (dataset.length == 0) {
			...
			this.alertMessage =
				"Ensure that the dataset is created before attempting to make any edits, as it currently does not exist.";
			this.editorJsonlFormGroup.disable({ emitEvent: false });
			return;
		}
		...
		const sourceValueChanges$ = () => {
			if (this.baseModel == "GOOGLE-PALM2-TEXT-BISON") {
				return this.googlePalm2TextBison.controls.map((form) =>
					merge(form.controls.inputText.valueChanges, form.controls.outputText.valueChanges)
				);
			}
			return this.openaiGpt3.controls.map((form) =>
				merge(
		...
		this.#datasetSubscription = merge(...sourceValueChanges$())
			.pipe(
				switchMap(() =>
					defer(() => {
						if (this.baseModel == "GOOGLE-PALM2-TEXT-BISON") {
							return this.googlePalm2TextBison.valueChanges as Observable<
								Required<Array<EditorGooglePalm2TextBisonRawValue>>
							>;
						}
				...
				switchMap((dataset) => this.#datasetService.replaceDataset(this.baseModel, name, dataset).pipe(map(() => null)))
			)
			.subscribe({
				next: () => {
					this.editorJsonlFormGroup.updateValueAndValidity({
	...
	private get googlePalm2ChatBison() {
		return this.editorJsonlFormGroup.get("googlePalm2ChatBison") as FormArray<
			FormGroup<EditorGooglePalm2ChatBisonFormControls>
		>;
	}

	private get googlePalm2TextBison() {
		return this.editorJsonlFormGroup.get("googlePalm2TextBison") as FormArray<
			FormGroup<EditorGooglePalm2TextBisonFormControls>
		>;
	}
