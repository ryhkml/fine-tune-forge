...
export function scanImage(files: Array<Express.Multer.File>) {
	const { GCP_PROJECT_ID, GCP_DOC_AI_PROCESSOR_CLIENT_EMAIL, GCP_DOC_AI_PROCESSOR_PRIVATE_KEY, GCP_DOC_AI_PROCESSOR_NAME } =
		env.getValue();
	...
	const sourceIProcessReq$ = files.length == 1 ? iProcessReq$ : forkJoin(bulkIProcessReq$);
	return sourceIProcessReq$.pipe(
		switchMap((payloads) => {
			...
			if (payloads.length == 1) {
				return defer(() => client.processDocument(payloads[0])).pipe(mapGetText(), toArray());
			}
			const bulkProcessDocument$ = payloads.map((payload) => {
				return defer(() => client.processDocument(payload)).pipe(mapGetText());
			});
